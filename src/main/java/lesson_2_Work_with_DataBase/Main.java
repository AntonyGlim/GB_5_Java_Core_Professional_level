...
//TODO
//В программе реализованы все пункты задания.
//В программе используется костыль, для защиты программы от вылетаний, а именно от запросов вида:
//"цена" (без последующего текста и пробелов) (и др. запросы) Ничего лучше я придумать не смог.
//Может быть есть решение проще.
//Суть костыля:
//Все запросы поступающие от пользователя сплитятся по пробелам в массив,
//это позволяет вычленять нужные нам данные и отбрасывать лишние.
//Например запрос "сменитьцену 10 20 куплю гараж не дорого" просплититься на 4 элемента
//[сменитьцену, 10, 20, куплю гараж не дорого] последний элемент использоваться не будет, и просто отброситься.
//Элементы [1] и [2] - уйдут в соответствующий метод.
//Проблемы возникали тогда, когда запрос имел вид: "сменитьцену".
//Тогда в методы попадали пустые значения и вылетали ошибки.
//Решено это было тем, что был создан костыльный массив, который изначально заполнен пробелами.
//И именно он дозаполняется введенными пользователем данными, и каждую итерацию обнуляется.
